...
export class Parser {
  ...
  parseExpression(expr, position) {
    ...
    if (expr[parseResult.position] == ')') {
      ...
      //Do not consume bracket from an enclosing expression
    ...
    if (parseResult.position != expr.length) {
      ...
      parseResult = this.parseApplication(expr, leftPosition, leftTerm);
    }

    //TODO: Re-factor duplication with reading a bracket earlier
    /*
     * Consume closing bracket
     */
    if (expr[parseResult.position] == ')') {
