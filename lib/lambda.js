...
export class Func extends Term {
  ...
  findLeftMostRedex(parentTerm) {
    return this.body.findLeftMostRedex(this);
  }
...
export class Application extends Term {
  ...
  findLeftMostRedex(parentTerm) {
    let redex, redexParent;

    if (this.left instanceof Func) {
      [redex, redexParent] = [this, parentTerm];
    } else {
      [redex, redexParent] = this.left.findLeftMostRedex(this) || this.right.findLeftMostRedex(this);
    }
    return [redex, redexParent];
  }
...
export class Interpreter {
  ...
  eval(term) {
    ...
    let [redex, redexParent] = term.findLeftMostRedex(null);

    //TODO: Re-factor
    while(redex && (reductionCount <= MAX_REDUCTIONS)) {
      const reduced = redex.reduce();

      if (redexParent) {
        //Replace the reference in the parent
        if (redexParent.left === redex) {
          redexParent.left = reduced;
        } else if (redexParent.right === redex) {
          redexParent.right = reduced;
        } else if (redexParent.body === redex) {
          redexParent.body = reduced;
        }
      } else {
        //Redex is the root term, replace it
        term = reduced;
      }
      reductionCount++;
      [redex, redexParent] = term.findLeftMostRedex(null);
    }
