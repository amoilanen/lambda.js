...
export class Parser {
  ...
  parseExpression(expr, position) {
    //console.log('parseExpression', expr, position);
    if (position === expr.length) {
      return {term: null, position};
    }

    const ch = expr[position];
    const chAhead = expr[position + 1];
    let parseResult;
     ...
     */
    while (expr[parseResult.position] == ')') {
    ...
    if (parseResult.position != expr.length) {
      const leftTerm = parseResult.term;
      const leftPosition = parseResult.position;

      return this.parseApplication(expr, leftPosition, leftTerm);
    }
  ...
  parseVariable(expr, position) {
    //console.log('parseVariable', expr, position);
    const ch = expr[position];
  ...
  parseFunction(expr, position) {
    //console.log('parseFunction', expr, position);
  ...
  parseApplication(expr, leftPosition, leftTerm) {
    //console.log('parseApplication', expr, leftPosition);
